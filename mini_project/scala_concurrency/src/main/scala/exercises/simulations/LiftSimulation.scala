...
object LiftSimulation extends App {
  val numFloors = 3
  val numLifts = 3
  val lifts = new Array[Lift](numLifts)
  val floors = new Array[Floor](numFloors)//its an array of type floor
  ...
  class Floor(id: Int) extends Thread{
    ...
    override def run(): Unit = {//thread starts with run method
      while(true) {
  ...
  class Lift(name: String) extends Thread {
    ...
    val requests = new mutable.Queue[Int]()     //multiple floor share request object to communicate with this.
    val lock = new Object
    ...
    var passengers = new mutable.Queue[Passenger]()//all lifts has its own passenger queue
  ...
  lift2.start();
  lift3.start();
  floor0.start();floor1.start();floor2.start();
